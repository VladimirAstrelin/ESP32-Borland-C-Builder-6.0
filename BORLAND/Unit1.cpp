//---------------------------------------------------------------------------
// ФАЙЛ: Unit1.cpp
// ПРОГРАММА: ESP32 Button Launcher v2.0
//
// НАЗНАЧЕНИЕ:
...
{
    Caption = "ESP32 Button Launcher v2.0";

    // --- Начальные тексты ---
    LBL_CONNECTION_STATUS->Caption = "CONNECTION STATUS: DISCONNECTED";
    LBL_CALC_STATUS->Caption       = "D4:  STATUS:";
    LBL_PAINT_STATUS->Caption      = "D16: STATUS:";
    LBL_D17_STATUS->Caption        = "D17: STATUS:";
    ...
    EDIT_D17->ReadOnly = true;   // новое поле
    EDIT_D4->Color  = clBtnFace;
    EDIT_D16->Color = clBtnFace;
    EDIT_D17->Color = clBtnFace; // новое поле
    ...
    UpdateD17PathDisplay();  // новая кнопка
...
void TForm1::ParseData(AnsiString data)
{
    ...
    // ================================================================
    // КНОПКА D4
    // ESP32 отправляет "BTN_D4:1" когда кнопка нажата
    // и "BTN_D4:0" когда отпущена
    // ================================================================
    if (data == "BTN_D4:1")
    {
        LBL_CALC_STATUS->Caption = "D4:  STATUS: PRESSED";
    ...
    if (data == "BTN_D4:0")
    {
        LBL_CALC_STATUS->Caption = "D4:  STATUS: released";
    ...
    if (data == "BTN_D16:1")
    {
        LBL_PAINT_STATUS->Caption = "D16: STATUS: PRESSED";
    ...
    if (data == "BTN_D16:0")
    {
        LBL_PAINT_STATUS->Caption = "D16: STATUS: released";
    ...
    if (data == "BTN_D17:1")
    {
        LBL_D17_STATUS->Caption = "D17: STATUS: PRESSED";
        SB_MAIN_STATUS_BAR->SimpleText = "D17 pressed - launching: " +
    ...
    if (data == "BTN_D17:0")
    {
        LBL_D17_STATUS->Caption = "D17: STATUS: released";
        SB_MAIN_STATUS_BAR->SimpleText = "D17 released";
        return;
    }
...
void TForm1::SetConnectedState(bool state, AnsiString port)
{
    ...
    else
    {
        ...
        // Сбрасываем статусы всех трёх кнопок
        LBL_CALC_STATUS->Caption  = "D4:  STATUS:";
        LBL_PAINT_STATUS->Caption = "D16: STATUS:";
        LBL_D17_STATUS->Caption   = "D17: STATUS:";
...
// Пользователь может открыть его в блокноте и посмотреть/изменить.
...
void TForm1::SaveSettings()
{
    ...
    // ВАЖНО: TIniFile записывает данные в файл только при delete
    delete ini;
}
...
// ====================================================================
//
// Используем WinExec как основной способ запуска.
// Если WinExec не справляется (возвращает <= 31) - пробуем ShellExecute.
// ShellExecute умнее: понимает ассоциации файлов, умеет запрашивать
// права администратора и работает с путями содержащими пробелы.
...
void TForm1::ExecuteD4Program()
{
    ...
    // Кавычки вокруг пути нужны если в пути есть пробелы:
    // "C:\Program Files\app.exe" - правильно
    // C:\Program Files\app.exe  - WinExec воспримет "Files\app.exe" как аргумент
...
// ExecuteD17Program - запускает программу назначенную на кнопку D17.
// Логика полностью аналогична D4 и D16.
...
// UpdateD17PathDisplay - аналогично D4 и D16, для кнопки D17
...
void __fastcall TForm1::BTN_CONNECTClick(TObject *Sender)
{
    ...
    // Медленные таймауты для CheckESP32 - ReadFile должен ждать ответа
    ...
    // Быстрые таймауты для рабочего режима - ReadFile возвращается мгновенно.
    // Комбинация MAXDWORD + 0 + 0: "есть данные - отдай, нет - сразу вернись"
    COMMTIMEOUTS workTO;
...
void __fastcall TForm1::BTN_DISCONNECTClick(TObject *Sender)
{
    ...
    if (hCom != INVALID_HANDLE_VALUE)
    {
        ...
        // чтобы не зависеть от флага commandPending
...
// BTN_PIN_TO_D17Click - обработчик кнопки PIN TO D17.
// Логика полностью аналогична D4 и D16.
